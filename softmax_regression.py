...
def compute_a(z):
    ...
    ## INSERT YOUR CODE HERE
    # from decimal import *
    # a = np.mat(np.zeros((len(z), len(z[0]))))
    # ez = np.mat(np.zeros((len(z), len(z[0]))), dtype=np.dtype(decimal))
    # esum = 0
    # for i in range(len(a)):
    #
    #     ez[i] = decimal(z[i,0]).exp()
    #
    #     esum += ez[i]
    ...
    a = np.mat(np.zeros((len(z),len(z[0]))))
    e_i = np.mat(np.zeros((len(z),len(z[0]))), dtype=np.dtype(Decimal))
    e_sum = 0
    for i in range(len(z)):
        e_i[i] = Decimal(z[i,0]).exp()
        e_sum = e_sum + e_i[i]
...
def backward(x,y,a):
    ...
    dz_dW = compute_dz_dW(x,len(da_dz))
    dz_db = compute_dz_db(len(da_dz))
    #########################################
